...
.belen-widget{position:fixed;right:12px;bottom:78px;z-index:85;font-family:Arial,sans-serif}.belen-launcher{position:relative;width:104px;height:126px;padding:0;border:0;border-radius:0;background:transparent;cursor:pointer;filter:drop-shadow(0 7px 10px rgba(0,0,0,.22));transition:transform .2s}.belen-launcher:hover{transform:translateY(-3px) scale(1.03)}.belen-launcher:focus-visible{outline:3px solid #25d366;outline-offset:3px}.belen-launcher img{display:block;width:100%;height:100%;object-fit:cover;object-position:50% 15%;border-radius:52px 52px 18px 18px;border:3px solid #fff;box-sizing:border-box}.belen-launcher:after{content:'💬';position:absolute;right:-3px;top:-5px;width:26px;height:26px;display:grid;place-items:center;background:#25d366;color:#fff;border:2px solid #fff;border-radius:50%;font-size:13px}.belen-nudge{position:absolute;right:92px;bottom:92px;width:170px;padding:9px 11px;border:2px solid #173f48;border-radius:15px;background:#fff;color:#173f48;box-shadow:3px 4px 0 rgba(23,63,72,.16);font-size:11px;font-weight:800;line-height:1.25;text-align:center}.belen-nudge:after{content:'';position:absolute;right:-9px;bottom:11px;width:13px;height:13px;background:#fff;border-right:2px solid #173f48;border-top:2px solid #173f48;transform:skew(16deg) rotate(27deg)}.belen-panel{display:none;position:absolute;right:0;bottom:136px;width:min(350px,calc(100vw - 26px));height:min(470px,calc(100vh - 180px));overflow:hidden;border:1px solid #c8dfe2;border-radius:20px;background:#f7fbfb;box-shadow:0 18px 60px rgba(15,45,53,.3)}.belen-panel.is-open{display:flex;flex-direction:column;animation:belen-in .22s ease-out}@keyframes belen-in{from{opacity:0;transform:translateY(12px) scale(.97)}to{opacity:1;transform:none}}.belen-head{display:flex;align-items:center;gap:9px;padding:11px 13px;background:linear-gradient(135deg,#173f48,#236875);color:#fff}.belen-head img{width:42px;height:42px;object-fit:cover;object-position:50% 15%;border-radius:50%;border:2px solid rgba(255,255,255,.85)}.belen-head strong{display:block;font-size:14px}.belen-head span{display:block;margin-top:2px;color:#c9e8df;font-size:10px}.belen-close{margin-left:auto;width:28px;height:28px;border:1px solid rgba(255,255,255,.7);border-radius:50%;background:transparent;color:#fff;font-size:18px;line-height:1;cursor:pointer}.belen-messages{flex:1;min-height:0;overflow:auto;padding:13px 11px 7px;background:linear-gradient(#fff,#f7fbfb)}.belen-bubble{width:fit-content;max-width:88%;margin:0 0 9px;padding:9px 11px;border-radius:16px 16px 16px 5px;background:#e7f4f5;color:#173f48;font-size:12px;line-height:1.35}.belen-bubble.user{margin-left:auto;border-radius:16px 16px 5px 16px;background:#dcf7e5;color:#14532d}.belen-options{display:flex;gap:5px;overflow-x:auto;padding:5px 10px 8px;background:#fff;scrollbar-width:none}.belen-options::-webkit-scrollbar{display:none}.belen-options button{white-space:nowrap;border:1px solid #8bc5cc;border-radius:50px;background:#f2fbfb;color:#17606a;padding:7px 9px;font-size:10px;font-weight:800;cursor:pointer}.belen-form{display:flex;gap:6px;padding:8px 10px 10px;border-top:1px solid #d9eaec;background:#fff}.belen-form input{min-width:0;flex:1;border:1px solid #b9d4d8;border-radius:50px;padding:9px 11px;outline:none;font-size:12px}.belen-form input:focus{border-color:#2f95a2;box-shadow:0 0 0 3px rgba(47,149,162,.14)}.belen-form button{width:38px;height:38px;border:0;border-radius:50%;background:#25b463;color:#fff;font-size:17px;cursor:pointer}.belen-quote{display:block;width:100%;margin-top:5px;border:0;border-radius:10px;background:#25b463;color:#fff;padding:8px;font-size:11px;font-weight:900;cursor:pointer}
@media(max-width:620px){.location-hotspot{min-height:100%}.ruiz-modal-card{padding:15px}.ruiz-modal-card h2{font-size:19px}.ruiz-modal{padding:9px}.whatsapp-form-card{padding:17px}.whatsapp-form-grid{grid-template-columns:1fr}.whatsapp-form-field.full{grid-column:auto}.whatsapp-form-actions{justify-content:stretch}.whatsapp-form-actions button{flex:1}.belen-widget{right:7px;bottom:68px}.belen-launcher{width:88px;height:108px}.belen-nudge{right:78px;bottom:80px;width:150px;font-size:10px}.belen-panel{bottom:116px;width:min(340px,calc(100vw - 20px));height:min(455px,calc(100vh - 145px))}}
</style>
...
(function(){
  ...
  var belenMessages=document.getElementById('belenMessages');
  var belenInput=document.getElementById('belenInput');
  function toggleBelen(force){var open=typeof force==='boolean'?force:!belenPanel.classList.contains('is-open');if(open){belenPanel.classList.add('is-open');belenLauncher.setAttribute('aria-expanded','true');if(belenNudge)belenNudge.style.display='none';setTimeout(function(){if(belenInput)belenInput.focus()},80)}else{belenPanel.classList.remove('is-open');belenLauncher.setAttribute('aria-expanded','false')}}
  ...
  function belenAdd(text,who){var bubble=document.createElement('div');bubble.className='belen-bubble'+(who==='user'?' user':'');bubble.textContent=text;belenMessages.appendChild(bubble);belenMessages.scrollTop=belenMessages.scrollHeight}
  function belenAnswer(text){var t=text.toLowerCase();if(/precio|precios|costo|costos|valor/.test(t))return'Los precios principales son: color y blanco y negro $1.250 por faz, anillado $4.000 y A4 autoadhesivo $7.500. También tenemos precios de fotos y plastificado.';if(/foto|fotos|kodak|polaroid|mitsubishi|inkjet/.test(t))return'Trabajamos fotos Mitsubishi, Inkjet, Kodak y Polaroid. Por ejemplo, 10×15: Mitsubishi $5.000, Inkjet $4.000 y Kodak $5.500. Decime el tamaño y la cantidad para orientarte mejor.';if(/impres|color|blanco|negro|hoja|pdf/.test(t))return'Hacemos impresiones color y blanco y negro, libros en PDF, A4 autoadhesivo y otros trabajos. El precio general es $1.250 por faz.';if(/anill/.test(t))return'El anillado tiene un precio de $4.000. Si me indicás la cantidad de hojas y el tamaño, Belen te ayuda a preparar la consulta.';if(/archivo|adjunt|documento/.test(t))return'Podés seleccionar tu archivo en el formulario de WhatsApp. Al abrirse el chat, tocá el clip 📎 para adjuntarlo.';if(/horario|llamar|atienden|abierto/.test(t))return'Podés dejar tu horario preferido en el formulario y el equipo de Imprenta Ruiz se comunicará con vos.';if(/hola|buenas|belen/.test(t))return'¡Hola! Estoy para ayudarte con impresiones, fotos, anillado y presupuestos.';return'Puedo ayudarte con precios, impresiones, fotos, anillado o un presupuesto. También podés tocar “Pedir presupuesto” y completar tus datos.'}
  function belenAsk(text){if(!text)return;belenAdd(text,'user');setTimeout(function(){belenAdd(belenAnswer(text),'bot')},260)}
  document.querySelectorAll('[data-belen-text]').forEach(function(b){b.addEventListener('click',function(){belenAsk(b.getAttribute('data-belen-text'))})});
  document.querySelectorAll('.belen-quote').forEach(function(b){b.addEventListener('click',function(){toggleBelen(false);openWhatsAppForm()})});
  var belenForm=document.getElementById('belenChatForm');
  if(belenForm)belenForm.addEventListener('submit',function(e){e.preventDefault();var text=belenInput.value.trim();if(!text)return;belenInput.value='';belenAsk(text)});
